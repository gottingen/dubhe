...
namespace dubhe {
    ...
    (specified by dubhe::PipeType) and is associated with a callable to invoke
    by the pipeline scheduler.

    You need to use the template function, dubhe::make_data_pipe, to create
    a data pipe. The input and output types of a dubhe::DataPipe should be decayed types
    ...
    @code{.cpp}
    dubhe::make_data_pipe<int, std::string>(
    ...
    of dubhe::Pipeflow in the second argument to probe the runtime information
    for a stage task, such as its line number and token number:

    @code{.cpp}
    dubhe::make_data_pipe<int, std::string>(
      dubhe::PipeType::SERIAL,
      [](int& input, dubhe::Pipeflow& pf) {
    ...
    */
    ...
    class DataPipe {
      ...
      */
      ...
      You should use the helper function, dubhe::make_data_pipe,
      to create a DataPipe object, especially when you need dubhe::DataPipe
      ...
      */
      ...
      A data pipe can be either parallel (dubhe::PipeType::PARALLEL) or serial
      (dubhe::PipeType::SERIAL).
      */
      PipeType type() const {
        return _type;
    ...
    /**
    @brief function to construct a data pipe (dubhe::DataPipe)

    @tparam Input input data type
    @tparam Output output data type
    @tparam C callable type

    dubhe::make_data_pipe is a helper function to create a data pipe (dubhe::DataPipe)
    in a data-parallel pipeline (dubhe::DataPipeline).
    The first argument specifies the direction of the data pipe,
    either dubhe::PipeType::SERIAL or dubhe::PipeType::PARALLEL,
    ...
    The callable can additionally take a reference of dubhe::Pipeflow,
    ...
    */
    ...
    Similar to dubhe::Pipeline, a dubhe::DataPipeline is a composable graph object
    for users to create a <i>data-parallel pipeline scheduling framework</i>
    using a module task in a taskflow.
    The only difference is that dubhe::DataPipeline provides a data abstraction
    ...
    @code{.cpp}
    #include <dubhe/taskflow.hpp>
    #include <dubhe/algorithm/data_pipeline.hpp>
    ...
    int main() {
      ...
      dubhe::DataPipeline pl(num_lines,
        dubhe::make_data_pipe<void, int>(dubhe::PipeType::SERIAL, [&](dubhe::Pipeflow& pf) -> int{
          if(pf.token() == 5) {
            pf.stop();
            return 0;
          }
    ...
    */
    ...
    class DataPipeline {
      ...
      */
      ...
      The first pipe must define a serial direction (dubhe::PipeType::SERIAL)
    ...
    DataPipeline<Ps...>::DataPipeline(size_t num_lines, Ps&&... ps) :
      ...
      if(num_lines == 0) {
        DUBHE_THROW("must have at least one line");
      }

      if(std::get<0>(_pipes).type() != PipeType::SERIAL) {
        DUBHE_THROW("first pipe must be serial");
      }
    ...
    void DataPipeline<Ps...>::_on_pipe(Pipeflow& pf, Runtime&) {
      ...
      visit_tuple([&](auto&& pipe){
        ...
        if constexpr (std::is_invocable_v<callable_t, Pipeflow&>) {
          // [](dubhe::Pipeflow&) -> void {}, i.e., we only have one pipe
          if constexpr (std::is_void_v<output_t>) {
            pipe._callable(pf);
          // [](dubhe::Pipeflow&) -> output_t {}
          } else {
        ...
        // other pipes with pipeflow in the second argument
          ...
          // [](input_t&, dubhe::Pipeflow&) -> void {}
          if constexpr (std::is_void_v<output_t>) {
            pipe._callable(std::get<input_t>(_buffer[pf._line].data), pf);
          // [](input_t&, dubhe::Pipeflow&) -> output_t {}
          } else {
            _buffer[pf._line].data = pipe._callable(
    ...
    void DataPipeline<Ps...>::_build() {
      ...
      for(size_t l = 0; l < num_lines(); l++) {
        ...
        _tasks[l + 1] = fb.emplace([this, l] (dubhe::Runtime& rt) mutable {
