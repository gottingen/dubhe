...
namespace dubhe {
    ...
    Pipeflow can only be created privately by the dubhe::Pipeline and
    ...
    */
    class Pipeflow {
      ...
      */
      void stop() {
        if(_pipe != 0) {
          DUBHE_THROW("only the first pipe can stop the token");
        }
        _stop = true;
      }
      ...
      */
      void defer(size_t token) {
        if(_pipe != 0) {
          DUBHE_THROW("only the first pipe can defer the current scheduling token");
        }
    ...
    */
    ...
    @em parallel direction or @em serial direction (specified by dubhe::PipeType)
    and is coupled with a callable to invoke by the pipeline scheduler.
    The callable must take a referenced dubhe::Pipeflow object in the first argument:

    @code{.cpp}
    Pipe{PipeType::SERIAL, [](dubhe::Pipeflow&){}}
    @endcode
    ...
    */
    template <typename C = std::function<void(dubhe::Pipeflow&)>>
    class Pipe {
      ...
      */
      ...
      @param d pipe type (dubhe::PipeType)
      @param callable callable type

      The constructor constructs a pipe with the given direction
      (dubhe::PipeType::SERIAL or dubhe::PipeType::PARALLEL) and the given callable.
      The callable must take a referenced dubhe::Pipeflow object in the first argument.
      ...
      */
      ...
      @param type a dubhe::PipeType variable
      */
      ...
      @param callable a callable object constructible from std::function<void(dubhe::Pipeflow&)>
    ...
    dubhe::Pipeline pipeline(num_lines,
    ...
    dubhe::Task task = taskflow.composed_of(pipeline)
                            .name("pipeline");
    ...
    Internally, dubhe::Pipeline uses std::tuple to store the given sequence of pipes.
    ...
    If applications need to change these pipes, please use dubhe::ScalablePipeline.
    */
    template <typename... Ps>
    class Pipeline {
    ...
    <i>pipeline scheduling framework</i> using a module task in a taskflow.
    Unlike dubhe::Pipeline that instantiates all pipes upon the construction time,
    dubhe::ScalablePipeline allows variable assignments of pipes using range iterators.
    ...
    dubhe::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
    ...
    Each pipe has the same type of `%dubhe::Pipe<%std::function<void(%dubhe::Pipeflow&)>>`
    ...
    */
    ...
    void ScalablePipeline<P>::reset(P first, P last) {
      ...
      if(num_pipes == 0) {
        DUBHE_THROW("pipeline cannot be empty");
      }
