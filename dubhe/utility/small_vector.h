...
#if defined(__GNUC__)
  #define DUBHE_LIKELY(x) (__builtin_expect((x), 1))
  #define DUBHE_UNLIKELY(x) (__builtin_expect((x), 0))
#else
  #define DUBHE_LIKELY(x) (x)
  #define DUBHE_UNLIKELY(x) (x)
#endif
...
namespace dubhe { namespace detail {
...
public:
  void push_back(const T &Elt) {
    if (DUBHE_UNLIKELY(this->EndX >= this->CapacityX))
      this->grow();
...
void SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize) {
  ...
  size_t NewCapacity = size_t(dubhe::detail::NextCapacity(CurCapacity+2));
...
public:
  ...
  */
  ...
  //explicit SmallVector(const dubhe::iterator_range<RangeTy> &R)
  //    : SmallVectorImpl<T>(N) {
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(dubhe::SmallVectorImpl<T> &LHS, dubhe::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(dubhe::SmallVector<T, N> &LHS, dubhe::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
