...
class Executor {
  ...
  public:
    ...
    */
    ...
    @return a dubhe::Future that will holds the result of the execution

    The method creates an asynchronous task to launch the given
    function on the given arguments.
    Unlike std::async, the return here is a @em dubhe::Future that holds
    ...
    @code{.cpp}
    dubhe::Future<std::optional<int>> future = executor.async([](){
    ...
    */
    ...
    Unlike std::async, the return here is a dubhe::Future that holds
    an optional object to the result.
    If the asynchronous task is cancelled before it runs, the return is
    a @c std::nullopt, or the value returned by the callable.

    @code{.cpp}
    dubhe::Future<std::optional<int>> future = executor.named_async("name", [](){
    ...
    */
    ...
    /**
    @brief similar to dubhe::Executor::named_async but does not return a future object

    This member function is more efficient than dubhe::Executor::named_async
    and is encouraged to use when there is no data returned.
...
inline Executor::Executor(size_t N) :
  ...
  if(N == 0) {
    DUBHE_THROW("no cpu workers to execute taskflows");
  }
...
dubhe::Future<void> Executor::run_until(Taskflow& f, P&& p, C&& c) {
  ...
  if(empty || p()) {
    ...
    promise.set_value();
    _decrement_topology_and_notify();
    return dubhe::Future<void>(promise.get_future(), std::monostate{});
  }
