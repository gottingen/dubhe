...
#include <dubhe/utility/os.h>
#include <dubhe/utility/math.h>
#include <dubhe/utility/small_vector.h>
#include <dubhe/utility/serializer.h>
#include <dubhe/core/error.h>
#include <dubhe/core/declarations.h>
#include <dubhe/core/semaphore.h>
#include <dubhe/core/environment.h>
#include <dubhe/core/topology.h>
#include <dubhe/core/tsq.h>
...
class Runtime {
  ...
  Issues a dubhe::Runtime::corun_all to finish all spawned asynchronous tasks
  ...
  */
  ...
  taskflow.emplace([&](dubhe::Runtime& rt){
  ...
  */
  ...
  The difference to dubhe::Executor::async is that the created asynchronous task
  pertains to the runtime object.
  Applications can explicitly issue dubhe::Runtime::corun_all
  ...
  */
  ...
  This member function is more efficient than dubhe::Runtime::async
  ...
  */
  ...
  /**
  @brief similar to dubhe::Runtime::silent_async but the caller must be the worker of the runtime
  ...
  */
  ...
  A target can be one of the following forms:
    ...
    + a composable graph object with `dubhe::Graph& T::graph()` defined

  @code{.cpp}
  // co-run a subflow and wait until all tasks complete
  taskflow.emplace([](dubhe::Runtime& rt){
    rt.corun([](dubhe::Subflow& sf){
      dubhe::Task A = sf.emplace([](){});
      dubhe::Task B = sf.emplace([](){});
    }); 
  });
  
  // co-run a taskflow and wait until all tasks complete
  dubhe::Taskflow taskflow1, taskflow2;
  taskflow1.emplace([](){ std::cout << "running taskflow1\n"; });
  taskflow2.emplace([&](dubhe::Runtime& rt){
  ...
  Although dubhe::Runtime::corun blocks until the operation completes,
  ...
  @attention
  Only the worker of this dubhe::Runtime can issue corun.
  */
  template <typename T>
  void corun(T&& target);
  ...
  */
  ...
  Coruns all asynchronous tasks (dubhe::Runtime::async,
  dubhe::Runtime::silent_async) with other workers until all those
  ...
  @attention
  Only the worker of this dubhe::Runtime can issue dubhe::Runtime::corun_all.
  */
...
Task parameters can be specified in one of the following types:
  + dubhe::TaskParams: assign the struct of defined parameters
  + dubhe::DefaultTaskParams: assign nothing
...
class Node {
  ...
  struct Subflow {
    ...
    std::function<void(dubhe::Subflow&)> work;
    Graph subgraph;
  };
