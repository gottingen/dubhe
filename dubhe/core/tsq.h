...
#include <dubhe/utility/macros.h>
...
namespace dubhe {
    ...
    */
    ...
    @tparam DUBHE_MAX_PRIORITY maximum level of the priority
    ...
    */
    template <typename T, unsigned DUBHE_MAX_PRIORITY = static_cast<unsigned>(TaskPriority::MAX)>
    class TaskQueue {

      static_assert(DUBHE_MAX_PRIORITY > 0, "DUBHE_MAX_PRIORITY must be at least one");
      ...
      // decent performance.
      CachelineAligned<std::atomic<int64_t>> _top[DUBHE_MAX_PRIORITY];
      CachelineAligned<std::atomic<int64_t>> _bottom[DUBHE_MAX_PRIORITY];
      std::atomic<Array*> _array[DUBHE_MAX_PRIORITY];
      std::vector<Array*> _garbage[DUBHE_MAX_PRIORITY];
      ...
      public:
        ...
        */
        DUBHE_FORCE_INLINE void push(T item, unsigned priority);
        ...
        */
        ...
        The return can be a @c nullptr if this operation failed (empty queue).
        */
        DUBHE_FORCE_INLINE T pop(unsigned priority);
      ...
      private:
        DUBHE_NO_INLINE Array* resize_array(Array* a, unsigned p, std::int64_t b, std::int64_t t);
    };

    // Constructor
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    TaskQueue<T, DUBHE_MAX_PRIORITY>::TaskQueue(int64_t c) {
      assert(c && (!(c & (c-1))));
      unroll<0, DUBHE_MAX_PRIORITY, 1>([&](auto p){
    ...
    // Destructor
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    TaskQueue<T, DUBHE_MAX_PRIORITY>::~TaskQueue() {
      unroll<0, DUBHE_MAX_PRIORITY, 1>([&](auto p){
    ...
    // Function: empty
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    bool TaskQueue<T, DUBHE_MAX_PRIORITY>::empty() const noexcept {
      for(unsigned i=0; i<DUBHE_MAX_PRIORITY; i++) {
        if(!empty(i)) {
          return false;
        }
      }
    ...
    // Function: empty
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    bool TaskQueue<T, DUBHE_MAX_PRIORITY>::empty(unsigned p) const noexcept {
    ...
    // Function: size
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    size_t TaskQueue<T, DUBHE_MAX_PRIORITY>::size() const noexcept {
      size_t s;
      unroll<0, DUBHE_MAX_PRIORITY, 1>([&](auto i) { s = i ? size(i) + s : size(i); });
      return s;
    }

    // Function: size
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    size_t TaskQueue<T, DUBHE_MAX_PRIORITY>::size(unsigned p) const noexcept {
      int64_t b = _bottom[p].data.load(std::memory_order_relaxed);
      int64_t t = _top[p].data.load(std::memory_order_relaxed);
    ...
    // Function: push
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    DUBHE_FORCE_INLINE void TaskQueue<T, DUBHE_MAX_PRIORITY>::push(T o, unsigned p) {
    ...
    // Function: pop
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    T TaskQueue<T, DUBHE_MAX_PRIORITY>::pop() {
      for(unsigned i=0; i<DUBHE_MAX_PRIORITY; i++) {
        if(auto t = pop(i); t) {
          return t;
        }
    ...
    // Function: pop
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    DUBHE_FORCE_INLINE T TaskQueue<T, DUBHE_MAX_PRIORITY>::pop(unsigned p) {
    ...
    // Function: steal
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    T TaskQueue<T, DUBHE_MAX_PRIORITY>::steal() {
      for(unsigned i=0; i<DUBHE_MAX_PRIORITY; i++) {
        if(auto t = steal(i); t) {
          return t;
        }
    ...
    // Function: steal
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    T TaskQueue<T, DUBHE_MAX_PRIORITY>::steal(unsigned p) {
    ...
    // Function: capacity
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    int64_t TaskQueue<T, DUBHE_MAX_PRIORITY>::capacity() const noexcept {
      size_t s;
      unroll<0, DUBHE_MAX_PRIORITY, 1>([&](auto i) {
    ...
    // Function: capacity
    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    int64_t TaskQueue<T, DUBHE_MAX_PRIORITY>::capacity(unsigned p) const noexcept {
      return _array[p].load(std::memory_order_relaxed)->capacity();
    }

    template <typename T, unsigned DUBHE_MAX_PRIORITY>
    DUBHE_NO_INLINE typename TaskQueue<T, DUBHE_MAX_PRIORITY>::Array*
      TaskQueue<T, DUBHE_MAX_PRIORITY>::resize_array(Array* a, unsigned p, std::int64_t b, std::int64_t t) {
