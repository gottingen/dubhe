...
#include <dubhe/algorithm/partitioner.h>
...
namespace dubhe {
    ...
    from which dubhe::Taskflow and dubhe::Subflow are derived.
    ...
    */
    class FlowBuilder {
      ...
      */
      ...
      @return a dubhe::Task handle

      The following example creates a static task.

      @code{.cpp}
      dubhe::Task static_task = taskflow.emplace([](){});
      @endcode
      ...
      */
      ...
      @tparam C callable type constructible from std::function<void(dubhe::Subflow&)>

      @param callable callable to construct a dynamic task

      @return a dubhe::Task handle

      The following example creates a dynamic task (dubhe::Subflow)
      that spawns two static tasks.

      @code{.cpp}
      dubhe::Task dynamic_task = taskflow.emplace([](dubhe::Subflow& sf){
        dubhe::Task static_task1 = sf.emplace([](){});
        dubhe::Task static_task2 = sf.emplace([](){});
      });
      @endcode
      ...
      */
      ...
      @tparam C callable type constructible from
              std::function<dubhe::SmallVector<int>()>
      ...
      */
      ...
      @code{.cpp}
      dubhe::Task A = taskflow.emplace([](){ std::cout << "A"; });
      dubhe::Task B = taskflow.emplace([](){ std::cout << "B"; });
      dubhe::Task C = taskflow.emplace([](){ std::cout << "C"; });
      dubhe::Task D = taskflow.emplace([](){ std::cout << "D"; });
      A.precede(B, C, D);
      ...
      */
      ...
      @code{.cpp}
      dubhe::Taskflow t1, t2;
      t1.emplace([](){ std::cout << "t1"; });

      // t2 is partially composed of t1
      dubhe::Task comp = t2.composed_of(t1);
      dubhe::Task init = t2.emplace([](){ std::cout << "t2"; });
      init.precede(comp);
      @endcode
      ...
      type dubhe::Graph such that it can interact with the executor.
      For example:
      ...
      struct MyObj {
        dubhe::Graph graph;
        MyObj() {
          dubhe::FlowBuilder builder(graph);
          dubhe::Task task = builder.emplace([](){
            std::cout << "a task\n";  // static task
          });
        }
      ...
      MyObj obj;
      dubhe::Task comp = taskflow.composed_of(obj);
      @endcode
      ...
      */
      ...
      dubhe::Task placeholder = taskflow.placeholder();
      ...
      */
      ...
      dubhe::Task C = taskflow.emplace([](){ std::cout << "C"; });
      dubhe::Task D = taskflow.emplace([](){ std::cout << "D"; });
      std::vector<dubhe::Task> tasks {A, B, C, D}
      ...
      */
      ...
      @tparam P partitioner type (default dubhe::DefaultPartitioner)
      ...
      */
      ...
      @param part partitioning algorithm (default dubhe::DefaultPartitioner)
    ...
    dubhe::Subflow is a derived class from dubhe::Runtime with a specialized mechanism
    to manage the execution of a child graph.
    By default, a subflow automatically @em joins its parent node.
    You may explicitly join or detach a subflow by calling dubhe::Subflow::join
    or dubhe::Subflow::detach, respectively.
    ...
    dubhe::Task A = taskflow.emplace([](){}).name("A");
    dubhe::Task C = taskflow.emplace([](){}).name("C");
    dubhe::Task D = taskflow.emplace([](){}).name("D");

    // create a subflow graph (dynamic tasking)
    dubhe::Task B = taskflow.emplace([] (dubhe::Subflow& subflow) {
      dubhe::Task B1 = subflow.emplace([](){}).name("B1");
      dubhe::Task B2 = subflow.emplace([](){}).name("B2");
      dubhe::Task B3 = subflow.emplace([](){}).name("B3");
      B1.precede(B3);
      B2.precede(B3);
    }).name("B");
