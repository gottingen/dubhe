...
#include <dubhe/core/observer.h>
#include <dubhe/core/taskflow.h>
#include <dubhe/core/async_task.h>
...
namespace dubhe {
    ...
    dubhe::Task A = taskflow.emplace([] () { std::cout << "This is TaskA\n"; });
    dubhe::Task B = taskflow.emplace([] () { std::cout << "This is TaskB\n"; });
    dubhe::Task C = taskflow.emplace([] () { std::cout << "This is TaskC\n"; });

    // Build precedence between tasks
    A.precede(B, C);

    dubhe::Future<void> fu = executor.run(taskflow);
    ...
    */
    class Executor {
      ...
      */
      ...
      @param taskflow a dubhe::Taskflow object

      @return a dubhe::Future that holds the result of the execution

      This member function executes the given taskflow once and returns a dubhe::Future
      object that eventually holds the result of the execution.

      @code{.cpp}
      dubhe::Future<void> future = executor.run(taskflow);
      // do something else
      future.wait();
      @endcode
      ...
      */
      dubhe::Future<void> run(Taskflow& taskflow);

      /**
      @brief runs a moved taskflow once

      @param taskflow a moved dubhe::Taskflow object

      @return a dubhe::Future that holds the result of the execution

      This member function executes a moved taskflow once and returns a dubhe::Future
      object that eventually holds the result of the execution.
      The executor will take care of the lifetime of the moved taskflow.

      @code{.cpp}
      dubhe::Future<void> future = executor.run(std::move(taskflow));
      // do something else
      future.wait();
      @endcode
      ...
      */
      ...
      This member function returns a dubhe::Future object that
      eventually holds the result of the execution.

      @code{.cpp}
      dubhe::Future<void> future = executor.run(taskflow, [](){ std::cout << "done"; });
      // do something else
      future.wait();
      @endcode
      ...
      */
      template<typename C>
      dubhe::Future<void> run(Taskflow& taskflow, C&& callable);
      ...
      @code{.cpp}
      dubhe::Future<void> future = executor.run(
      ...
      */
      ...
      This member function executes the given taskflow @c N times and returns a dubhe::Future
      object that eventually holds the result of the execution.

      @code{.cpp}
      dubhe::Future<void> future = executor.run_n(taskflow, 2);  // run taskflow 2 times
      // do something else
      future.wait();
      @endcode
      ...
      */
      dubhe::Future<void> run_n(Taskflow& taskflow, size_t N);
      ...
      This member function executes a moved taskflow @c N times and returns a dubhe::Future
      object that eventually holds the result of the execution.
      The executor will take care of the lifetime of the moved taskflow.

      @code{.cpp}
      dubhe::Future<void> future = executor.run_n(
      ...
      */
      ...
      @param taskflow a dubhe::Taskflow
      @param N number of runs
      @param callable a callable object to be invoked after this run
      ...
      */
      template<typename C>
      dubhe::Future<void> run_n(Taskflow& taskflow, size_t N, C&& callable);

      /**
      @brief runs a moved taskflow for @c N times and then invokes a callback

      @param taskflow a moved dubhe::Taskflow
      @param N number of runs
      @param callable a callable object to be invoked after this run
      ...
      */
      ...
      @code{.cpp}
      dubhe::Future<void> future = executor.run_until(
      ...
      */
      template<typename P>
      dubhe::Future<void> run_until(Taskflow& taskflow, P&& pred);
      ...
      */
      ...
      dubhe::Future<void> run_until(Taskflow& taskflow, P&& pred, C&& callable);
      ...
      */
      ...
      @tparam T target type which has `dubhe::Graph& T::graph()` defined
      @param target the target task graph object

      The method runs a target graph which has `dubhe::Graph& T::graph()` defined
      and waits until the execution completes.
      Unlike the typical flow of calling `dubhe::Executor::run` series
      ...
      dubhe::Taskflow taskflow;
      std::array<dubhe::Taskflow, 1000> others;
      ...
      @attention
      You must call dubhe::Executor::corun from a worker of the calling executor
      or an exception will be thrown.
      */
      template <typename T>
      void corun(T& target);
      ...
      @attention
      You must call dubhe::Executor::corun_until from a worker of the calling executor
      or an exception will be thrown.
      */
      template <typename P>
      ...
      */
      ...
      @code{.cpp}
      dubhe::Executor executor(4);   // 4 workers in the executor
      ...
      */
      ...
      @tparam Observer observer type derived from dubhe::ObserverInterface
      ...
      dubhe::ObserverInterface::on_entry and dubhe::ObserverInterface::on_exit
      ...
      */
      ...
      /**
      @brief similar to dubhe::Executor::async but does not return a future object
      ...
      This member function is more efficient than dubhe::Executor::async
      ...
      */
      ...
      @tparam F callable type
      @tparam Tasks task types convertible to dubhe::AsyncTask

      @param func callable object
      @param tasks asynchronous tasks on which this execution depends

      @return a dubhe::AsyncTask handle

      This member function is more efficient than dubhe::Executor::dependent_async
      ...
      @code{.cpp}
      dubhe::AsyncTask A = executor.silent_dependent_async([](){ printf("A\n"); });
      dubhe::AsyncTask B = executor.silent_dependent_async([](){ printf("B\n"); });
      ...
      */
      ...
      >
      dubhe::AsyncTask silent_dependent_async(F&& func, Tasks&&... tasks);
      ...
      @code{.cpp}
      dubhe::AsyncTask A = executor.silent_dependent_async("A", [](){ printf("A\n"); });
      dubhe::AsyncTask B = executor.silent_dependent_async("B", [](){ printf("B\n"); });
      ...
      */
      ...
      >
      dubhe::AsyncTask silent_dependent_async(P&& params, F&& func, Tasks&&... tasks);
      ...
      @code{.cpp}
      std::array<dubhe::AsyncTask, 2> array {
      ...
      */
      ...
      >
      dubhe::AsyncTask silent_dependent_async(F&& func, I first, I last);
      ...
      */
      ...
      >
      dubhe::AsyncTask silent_dependent_async(P&& params, F&& func, I first, I last);
      ...
      @return a pair of a dubhe::AsyncTask handle and
                        a @std_future that holds the result of the execution

      The example below creates three asynchronous tasks, @c A, @c B, and @c C,
      in which task @c C runs after task @c A and task @c B.
      Task @c C returns a pair of its dubhe::AsyncTask handle and a std::future<int>
      that eventually will hold the result of the execution.
      ...
      You can mixed the use of dubhe::AsyncTask handles
      ...
      */
      ...
      void _process_async_dependent(Node*, dubhe::AsyncTask&, size_t&);
    ...
    inline Executor::Executor(size_t N) :
      ...
      if(N == 0) {
        DUBHE_THROW("executor must define at least one worker");
      }

      _spawn(N);

      // initialize the default observer if requested
      if(has_env(DUBHE_ENABLE_PROFILER)) {
    ...
    inline void Executor::_invoke_static_task(Worker& worker, Node* node) {
      ...
      DUBHE_EXECUTOR_EXCEPTION_HANDLER(worker, node, {
    ...
    inline void Executor::_invoke_subflow_task(Worker& w, Node* node) {
      ...
      DUBHE_EXECUTOR_EXCEPTION_HANDLER(w, node, {
    ...
    // Function: run
    inline dubhe::Future<void> Executor::run(Taskflow& f) {
      return run_n(f, 1, [](){});
    }
    ...
    // Function: run
    template <typename C>
    dubhe::Future<void> Executor::run(Taskflow& f, C&& c) {
    ...
    // Function: run_n
    inline dubhe::Future<void> Executor::run_n(Taskflow& f, size_t repeat) {
    ...
    // Function: run_n
    template <typename C>
    dubhe::Future<void> Executor::run_n(Taskflow& f, size_t repeat, C&& c) {
      return run_until(
    ...
    // Function: run_until
    template<typename P>
    dubhe::Future<void> Executor::run_until(Taskflow& f, P&& pred) {
    ...
    // Function: run_until
    template <typename P, typename C>
    dubhe::Future<void> Executor::run_until(Taskflow& f, P&& p, C&& c) {
      ...
      if(empty || p()) {
        ...
        promise.set_value();
        _decrement_topology();
        return dubhe::Future<void>(promise.get_future());
      }
      ...
      dubhe::Future<void> future(t->_promise.get_future(), t);
    ...
    // Function: run_until
    template <typename P, typename C>
    dubhe::Future<void> Executor::run_until(Taskflow&& f, P&& pred, C&& c) {
    ...
    void Executor::corun(T& target) {
      ...
      if(w == nullptr) {
        DUBHE_THROW("corun must be called by a worker of the executor");
      }
    ...
    void Executor::corun_until(P&& predicate) {
      ...
      if(w == nullptr) {
        DUBHE_THROW("corun_until must be called by a worker of the executor");
      }
    ...
    inline void Subflow::join() {
      ...
      if(!_joinable) {
        DUBHE_THROW("subflow not joinable");
      }
    ...
    inline void Subflow::detach() {
      ...
      if(!_joinable) {
        DUBHE_THROW("subflow already joined or detached");
      }
