...
void starvation_loop_test(size_t W) {
  ...
  for(size_t b=0; b<B; b++) {
    dubhe::Task task = taskflow.emplace([&](){
      // record the threads
      {
...
void subflow_starvation_test(size_t W) {
  ...
  taskflow.emplace([&](dubhe::Subflow& subflow){
    ...
    // fork
    for(size_t b=0; b<B; b++) {
      dubhe::Task task = subflow.emplace([&](){
        // record the threads
        {
          std::scoped_lock lock(mutex);
          set.insert(executor.this_worker_id());
...
void embarrasing_starvation_test(size_t W) {
  ...
  dubhe::Taskflow taskflow, parent;
  dubhe::Executor executor(W);
...
void skewed_starvation(size_t W) {
  ...
  dubhe::Task parent = taskflow.emplace([&](){
    set.clear();
    count.store(0, std::memory_order_relaxed);
    stop.store(false, std::memory_order_relaxed);
  }).name("root");

  dubhe::Task left, right;
...
void nary_starvation(size_t W) {
  ...
  for(size_t w=0; w<W; w++) {
    ...
    for(size_t i=0; i<N; i++) {
      dubhe::Task task = taskflow.emplace([&, w, p, i](){
...
void wavefront_starvation(size_t W) {
  ...
  std::vector<std::vector<dubhe::Task>> G;
  G.resize(W);
