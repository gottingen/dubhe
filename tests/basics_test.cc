...
TEST_CASE("Type" * doctest::timeout(300)) {
  ...
  dubhe::Taskflow taskflow, taskflow2;

  auto t1 = taskflow.emplace([](){});
  auto t2 = taskflow.emplace([](){ return 1; });
  auto t3 = taskflow.emplace([](dubhe::Subflow&){ });
  auto t4 = taskflow.composed_of(taskflow2);
  auto t5 = taskflow.emplace([](){ return dubhe::SmallVector{1, 2}; });
  auto t6 = taskflow.emplace([](dubhe::Runtime&){});
  auto t7 = taskflow.emplace([](dubhe::Runtime&){ return 1; });
  auto t8 = taskflow.emplace([](dubhe::Runtime&){ return dubhe::SmallVector{1, 2}; });

  REQUIRE(t1.type() == dubhe::TaskType::STATIC);
  REQUIRE(t2.type() == dubhe::TaskType::CONDITION);
  REQUIRE(t3.type() == dubhe::TaskType::SUBFLOW);
  REQUIRE(t4.type() == dubhe::TaskType::MODULE);
  REQUIRE(t5.type() == dubhe::TaskType::CONDITION);
  REQUIRE(t6.type() == dubhe::TaskType::STATIC);
  REQUIRE(t7.type() == dubhe::TaskType::CONDITION);
  REQUIRE(t8.type() == dubhe::TaskType::CONDITION);
}
...
TEST_CASE("Builder" * doctest::timeout(300)) {
  ...
  std::vector<dubhe::Task> silent_tasks;
...
TEST_CASE("Iterators" * doctest::timeout(300)) {
  ...
  SUBCASE("Order") {
    ...
    A.for_each_successor([&, i=0] (dubhe::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == B);
        break;
    ...
    E.for_each_dependent([&, i=0](dubhe::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == A);
        break;
  ...
  SUBCASE("Generic") {
    ...
    taskflow.for_each_task([&tasks](dubhe::Task s){
      tasks.push_back(s);
    });
    ...
    A.for_each_successor([B](dubhe::Task s){ REQUIRE(s==B); });
    B.for_each_dependent([A](dubhe::Task s){ REQUIRE(s==A); });
    ...
    A.for_each_successor([&](dubhe::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
    ...
    counter = a = b = c = d = e = 0;
    B.for_each_dependent([&](dubhe::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
...
void sequential_runs(unsigned W) {
  ...
  SUBCASE("RunWithFuture") {
    ...
    auto B = f.emplace([&](dubhe::Subflow& subflow){
      count ++;
    ...
    std::list<dubhe::Future<void>> fu_list;
  ...
  SUBCASE("MultipleRuns") {
    ...
    dubhe::Taskflow tf1, tf2, tf3, tf4;
...
TEST_CASE("RunAndWait.Simple") {
  ...
  dubhe::Taskflow others;
  dubhe::Task A = others.emplace([&](){ counter++; });
  dubhe::Task B = others.emplace([&](){ counter++; });
  A.precede(B);
...
void worker_id(unsigned w) {
  ...
  for(int i=0; i<1000; i++) {
    ...
    auto B = taskflow.emplace([&](dubhe::Subflow& sf){
      auto id = executor.this_worker_id();
      REQUIRE(id>=0);
      REQUIRE(id< w);
      sf.emplace([&](){
...
void parallel_runs(unsigned w) {
  ...
  auto make_taskflow = [&] (dubhe::Taskflow& tf) {
  ...
  SUBCASE("RunAndWaitForAll") {
    dubhe::Executor executor(w);
    counter = 0;
    std::vector<std::unique_ptr<dubhe::Taskflow>> taskflows(32);
    std::atomic<size_t> barrier(0);
    for(int t=0; t<32; t++) {
      threads.emplace_back([&, t=t] () {
        taskflows[t] = std::make_unique<dubhe::Taskflow>();
...
void observer(unsigned w) {
  ...
  auto observer = executor.make_observer<dubhe::ChromeObserver>();

  dubhe::Taskflow taskflowA;
  std::vector<dubhe::Task> tasks;
  // Static tasking
  for(auto i=0; i < 64; i ++) {
