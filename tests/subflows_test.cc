...
void joined_subflow(unsigned W) {
  ...
  SUBCASE("Trivial") {
    ...
    dubhe::Task subflow3, subflow3_;
    //std::future<int> fu3, fu3_;
    std::atomic<int> fu3v{0}, fu3v_{0};

    // empty flow
    auto subflow1 = tf.emplace([&] (dubhe::Subflow& fb) {
      fu3v++;
      fb.join();
    }).name("subflow1");

    // nested empty flow
    auto subflow2 = tf.emplace([&] (dubhe::Subflow& fb) {
      fu3v++;
      fb.emplace([&] (dubhe::Subflow& fb2) {
        fu3v++;
        fb2.emplace( [&] (dubhe::Subflow& fb3) {
          fu3v++;
          fb3.join();
        }).name("subflow2_1_1");
      }).name("subflow2_1");
    ...
    subflow3 = tf.emplace([&] (dubhe::Subflow& fb) {
      ...
      subflow3_ = fb.emplace([&] (dubhe::Subflow& fb2) {
        REQUIRE(fu3v_ == 3);
        fu3v++;
        fu3v_++;
        //return 200;
  ...
  SUBCASE("Complex") {
    ...
    auto B = tf.emplace([&count, &data, &sum](dubhe::Subflow& fb){
      ...
      auto n = fb.emplace([&count](dubhe::Subflow& fb2){
...
// --------------------------------------------------------
void detach_spawn(const int max_depth, std::atomic<int>& counter, int depth, dubhe::Subflow& subflow)  {
  if(depth < max_depth) {
    counter.fetch_add(1, std::memory_order_relaxed);
    subflow.emplace([&, max_depth, depth=depth+1](dubhe::Subflow& sfl){
      detach_spawn(max_depth, counter, depth, sfl); }
    );
    subflow.emplace([&, max_depth, depth=depth+1](dubhe::Subflow& sfr){
...
void join_spawn(const int max_depth, std::atomic<int>& counter, int depth, dubhe::Subflow& subflow)  {
  if(depth < max_depth) {
    counter.fetch_add(1, std::memory_order_relaxed);
    subflow.emplace([&, max_depth, depth=depth+1](dubhe::Subflow& sfl){
...
void mix_spawn(
  const int max_depth, std::atomic<int>& counter, int depth, dubhe::Subflow& subflow
) {
...
TEST_CASE("TreeSubflow" * doctest::timeout(300)) {
  ...
  SUBCASE("AllDetach") {
    ...
    for(int W=1; W<=4; W++) {
      ...
      dubhe::Taskflow tf;
      tf.emplace([&](dubhe::Subflow& subflow){
...
// Testcase: FibSubflow
// --------------------------------------------------------
int fibonacci_spawn(int n, dubhe::Subflow& sbf) {
  if (n < 2) return n;
  int res1, res2;
  sbf.emplace([&res1, n] (dubhe::Subflow& sbfl) { res1 = fibonacci_spawn(n - 1, sbfl); } );
  sbf.emplace([&res2, n] (dubhe::Subflow& sbfr) { res2 = fibonacci_spawn(n - 2, sbfr); } );
