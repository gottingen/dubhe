...
void runtime_async_task_exception(unsigned W) {
  ...
  // runtime async
  auto A = taskflow.emplace([](dubhe::Runtime& rt){
  ...
  flag = 0;
  taskflow.clear();
  A = taskflow.emplace([&](dubhe::Runtime& rt){
...
void joined_subflow_exception_1(unsigned W) {
  ...
  taskflow.emplace([&] (dubhe::Subflow& sf0) {
    for (int i = 0; i < 16; ++i) {
      sf0.emplace([&] (dubhe::Subflow& sf1) {
        for (int j = 0; j < 16; ++j) {
          sf1.emplace([] () {
            throw std::runtime_error("x");
          });
...
void executor_corun_exception(unsigned W) {
  ...
  for(size_t i=0; i<100; i++) {
    taskflow1.emplace([](dubhe::Subflow& sf){
...
void runtime_corun_exception(unsigned W) {
  ...
  auto task = taskflow2.emplace([&](dubhe::Runtime& rt){
