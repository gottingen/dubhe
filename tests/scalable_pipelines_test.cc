...
void scalable_pipeline(size_t num_lines, size_t num_pipes) {
  ...
  for(size_t i=0; i<num_pipes; i++) {
    pipes.emplace_back(dubhe::PipeType::SERIAL, [&](dubhe::Pipeflow& pf) mutable {
  ...
  dubhe::ScalablePipeline spl(num_lines, pipes.begin(), pipes.end());
...
void scalable_pipeline_reset(size_t num_lines, size_t num_pipes) {
  ...
  dubhe::ScalablePipeline<typename decltype(pipes)::iterator> spl(num_lines);
...
void scalable_pipeline_lines_reset(size_t num_lines, size_t num_pipes) {
  ...
  std::vector<dubhe::Pipe<>> pipes;
  dubhe::ScalablePipeline<typename decltype(pipes)::iterator> spl;
...
void ifelse_spipeline(size_t L, unsigned w) {
  ...
  std::vector<dubhe::Pipe<>> pipes;
  dubhe::ScalablePipeline<typename decltype(pipes)::iterator> pl;
  ...
  for(size_t N = 1; N < maxN; ++N) {
    ...
    // pipe 1
    pipes.emplace_back(dubhe::PipeType::SERIAL, [&, N](auto& pf){
        if(pf.token() == N) {
          pf.stop();
          return;
        }
      ...
      // pipe 2
    pipes.emplace_back(dubhe::PipeType::PARALLEL, [&](auto& pf){
    ...
    // pipe 3
    pipes.emplace_back(dubhe::PipeType::SERIAL, [&](auto& pf){
...
void spipeline_in_spipeline(size_t L, unsigned w, unsigned subL) {
  ...
  for (size_t N = 1; N < maxN; ++N) {
    for(size_t subN = 1; subN < maxsubN; ++subN) {
      ...
      // begin of pipe 1 -----------------------------
      pipes.emplace_back(dubhe::PipeType::SERIAL, [&, N, subN, subL](auto& pf) mutable {
        if(j1 == N) {
          pf.stop();
          return;
        }
        ...
        std::vector<dubhe::Pipe<>> subpipes;
        dubhe::ScalablePipeline<typename decltype(subpipes)::iterator> subpl;

        // subpipe 1
        subpipes.emplace_back(dubhe::PipeType::SERIAL, [&, subN](auto& subpf) mutable {
            if(subj1 == subN) {
              subpf.stop();
              return;
            }
        ...
        // subpipe 2
        subpipes.emplace_back(dubhe::PipeType::PARALLEL, [&, subN](auto& subpf) mutable {
            REQUIRE(subj2++ < subN);
            REQUIRE(subpf.token() % subL == subpf.line());
            REQUIRE(
      ...
      //begin of pipe 2 ---------------------------
      pipes.emplace_back(dubhe::PipeType::PARALLEL, [&, subN, subL](auto& pf) mutable {
      ...
      // begin of pipe 4 ---------------------------
      pipes.emplace_back(dubhe::PipeType::SERIAL, [&, subN](auto& pf) mutable {
        int res = std::accumulate(
          source[j4].begin(),
...
void snig_spipeline(size_t L, unsigned w) {
  ...
  for(auto&& NUM_DEVICE: NUM_DEVICES) {
    ...
    std::vector<dubhe::Task> dev_ends(NUM_DEVICE);
    for(auto&& dev_end: dev_ends) {
      dev_end = taskflow.emplace([](){}).name("dev_end");
    }

    std::vector<dubhe::Task> first_fetches(NUM_DEVICE);
    std::vector<dubhe::Task> fetches(NUM_DEVICE);
    std::vector<std::vector<dubhe::Pipe<>>> pipes(NUM_DEVICE);

    // for type
    using pipeline_it = std::vector<dubhe::Pipe<>>::iterator;

    std::vector<dubhe::Task> module_of_pipelines(NUM_DEVICE);

    std::vector<dubhe::ScalablePipeline<pipeline_it>> pipelines(NUM_DEVICE);
    ...
    for(size_t dev = 0; dev < NUM_DEVICE; ++dev) {
      ...
      // pipe 2
      pipes[dev].emplace_back(
        dubhe::PipeType::PARALLEL, [&, dev, BATCH_SIZE](auto& pf) mutable {
      ...
      // pipe 3
      pipes[dev].emplace_back(
        dubhe::PipeType::SERIAL, [&, dev, BATCH_SIZE](auto& pf) mutable {
...
void spawn(
  ...
  std::vector<std::vector<dubhe::Pipe<>>>& pipes,
  std::vector<dubhe::ScalablePipeline<typename std::vector<dubhe::Pipe<>>::iterator>>& spls,
  size_t& counter
) {

  // construct pipes
  for(size_t p = 0; p < NUM_PIPES; ++p) {
    pipes[r].emplace_back(dubhe::PipeType::SERIAL, [&, maxN, r](dubhe::Pipeflow& pf) mutable {
  ...
  if(r + 1 < NUM_RECURS) {
    auto spawn_t = sf.emplace([&, L, NUM_PIPES, NUM_RECURS, maxN, r](dubhe::Subflow& sf2) mutable {
...
void subflow_spipeline(unsigned NUM_RECURS, unsigned w, size_t L) {
  ...
  dubhe::Executor executor(w);
  dubhe::Taskflow taskflow;
  std::vector<dubhe::ScalablePipeline<typename std::vector<dubhe::Pipe<>>::iterator>> spls(NUM_RECURS);
  std::vector<std::vector<dubhe::Pipe<>>> pipes(NUM_RECURS);
  ...
  auto subflows = taskflow.emplace([&, L, NUM_PIPES, NUM_RECURS, maxN](dubhe::Subflow& sf){
...
TEST_CASE("ScalablePipeline.move" * doctest::timeout(300)) {
  ...
  for(size_t i=0; i<N; i++) {
    pipes.emplace_back(dubhe::PipeType::SERIAL, [&](dubhe::Pipeflow& pf) {
      if (pf.token() == 5) {
        pf.stop();
      }
      else {
