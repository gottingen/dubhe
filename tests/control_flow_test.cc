...
TEST_CASE("Cond.Types") {
  ...
  static_assert(dubhe::is_condition_task_v<decltype(explicit_c)>);
  static_assert(dubhe::is_condition_task_v<decltype(implicit_c)>);

  REQUIRE(explicit_task.type() == dubhe::TaskType::CONDITION);
  REQUIRE(implicit_task.type() == dubhe::TaskType::CONDITION);
}
...
void conditional_spawn(
  ...
  const int max_depth,
  int depth,
  dubhe::Subflow& subflow
)  {
  if(depth < max_depth) {
    for(int i=0; i<2; i++) {
      auto A = subflow.emplace([&](){ counter++; });
      auto B = subflow.emplace(
        [&, max_depth, depth=depth+1](dubhe::Subflow& sf2){
...
TEST_CASE("BTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    for(int l=1; l<12; l++) {
      dubhe::Taskflow flow;
      std::vector<dubhe::Task> prev_tasks;
...
TEST_CASE("DynamicBTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    ...
    dubhe::Taskflow flow;
    flow.emplace([&](dubhe::Subflow& subflow) {
      counter++;
...
void nested_cond(unsigned w) {
  ...
  dubhe::Taskflow flow;
  auto S = flow.emplace([](){});
  auto A = flow.emplace([&] (dubhe::Subflow& subflow) mutable {
    //           ___________
    //          |           |
    //          v           |
    //   S1 -> A1 -> B1 -> cond
    ...
    auto B1 = subflow.emplace([&](dubhe::Subflow& sf){
...
void hierarchical_condition(unsigned w) {
  ...
  dubhe::Executor executor(w);
  dubhe::Taskflow tf0("c0");
  dubhe::Taskflow tf1("c1");
  dubhe::Taskflow tf2("c2");
  dubhe::Taskflow tf3("top");
  ...
  auto sbf1 = tf3.emplace([&](dubhe::Subflow& sbf){
...
void condition_subflow(unsigned W) {
  ...
  auto subflow = taskflow.emplace([&](dubhe::Subflow& sf){
    sf.emplace([&, i](){
      REQUIRE(i<I);
      data[i] = i*(i+1)/2*123;;
...
TEST_CASE("MultiCond.Types") {
  ...
  auto explicit_mc = [](){
    dubhe::SmallVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> dubhe::SmallVector<int> {
    return {1, 2, 3, 9};
  };
  ...
  static_assert(dubhe::is_multi_condition_task_v<decltype(explicit_mc)>);
  static_assert(dubhe::is_multi_condition_task_v<decltype(implicit_mc)>);
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> dubhe::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> dubhe::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> dubhe::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> dubhe::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> dubhe::SmallVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
